...
usage() {
  echo "$0 <INDEX_URL> <OUT_FILE> <SEGMENTS_OUT_DIR> <RM_SEGMENTS> <PARALLEL_FILES>
Params:
...
[[ -z "$OUT_FILE" ]] && >&2 echo "Output file not provided. Will only download the segment files"
...
while IFS= read -r line; do
  ...
  if (grep -oE '^#.*' <<<"$line" >/dev/null); then
    ...
    segment_name_out="$(printf 's%04d.ts' "$segment_indx")"
    echo "$segment_name_out" >>"$SEGMENTS_OUT_DIR/$mod_index_file"
    echo "$segment_url" >>"$SEGMENTS_OUT_DIR/$aria_dwnld_list"
    echo " out=$segment_name_out" >>"$SEGMENTS_OUT_DIR/$aria_dwnld_list"
    segment_indx=$((segment_indx + 1))
  fi
  #continue
...
echo "Download List: $SEGMENTS_OUT_DIR/$aria_dwnld_list "
aria2c -c -s 1 -j "${PARALLEL_FILES}" -x 1 -k 1M --console-log-level=warn -d "$SEGMENTS_OUT_DIR" --input-file "$SEGMENTS_OUT_DIR/$aria_dwnld_list" --download-result=hide
